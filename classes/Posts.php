...
class Posts
{
	...
	public function get_userids_for_forum($forumid, $limit)
	{
		$sql = "SELECT DISTINCT `userid`
			...
			LIMIT %d";
		$sql = WPF()->db->prepare($sql, $forumid, $limit);
		return array_map('wpforo_bigintval', WPF()->db->get_col($sql));
	}

	public function get_users_count_for_forum($forumid)
	{
		#$sql = "SELECT COUNT( DISTINCT `userid` ) AS quantity 
		#FROM `" . WPF()->tables->posts . "`
		#WHERE `forumid` = %d";
		$sql = "SELECT user_count AS quantity 
			FROM `wp_wpforo_posts_user_forum_cache`
			WHERE `forumid` = %d";
		$sql = WPF()->db->prepare($sql, $forumid);
		return (int) WPF()->db->get_var($sql);
	}

	public function get_userids_for_topic($topicid, $limit)
	{
		$sql = "SELECT DISTINCT `userid`
			FROM `" . WPF()->tables->posts . "`
			WHERE `topicid` = %d
			...
			ORDER BY `created` DESC
			LIMIT %d";
		$sql = WPF()->db->prepare($sql, $topicid, $limit);
		return array_map('wpforo_bigintval', WPF()->db->get_col($sql));
	}
